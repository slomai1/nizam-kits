...
const confirm = (args && args.confirm) || null;

if (!repo) {
  log(
    '⚠️ مطلوب: repo. استخدم: Workflow({name: "land-and-deploy", args: {repo: "owner/repo"}})',
  );
  return { status: "aborted", reason: "missing-repo" };
}

// ─── بوابة تأكيد ──────────────────────────────────────────────────
// هذا الـ workflow يدمج PR وينشر للإنتاج: فعلان خارجيان لا رجعة فيهما
// بسهولة. لا يُشغَّل بلا موافقة صريحة من المستخدم في نفس الاستدعاء.
if (confirm !== "نعم انشر") {
  log("⛔ **متوقف — يلزم تأكيد صريح**");
  log("");
  log(`هذا الـ workflow سيدمج PR في **${repo}** ثم ينشر إلى **production**.`);
  log("الدمج والنشر فعلان خارجيان يراهما الجمهور ولا يسهل التراجع عنهما.");
  log("");
  log("للمتابعة أعد الاستدعاء مع تأكيد صريح:");
  log("```");
  log(
    `Workflow({name: "land-and-deploy", args: {repo: "${repo}"${pullNumber ? `, pull_number: ${pullNumber}` : ""}, confirm: "نعم انشر"}})`,
  );
  log("```");
  return { status: "awaiting-confirmation", repo, pullNumber };
}

log(`🚀 Land & Deploy: **${repo}**`);
if (pullNumber) log(`PR: #${pullNumber}`);
log("✅ التأكيد مستلم — المتابعة");
...
**المخرجات:** هل تم الدمج؟ commit SHA؟ تحذيرات`,
  { label: "دمج PR" },
);
...
**المخرجات:** حالة CI النهائية — عدد checks الناجحة/الإجمالي — أي أخطاء`,
  { label: "فحص CI" },
);
...
**المخرجات:** رابط النشر — حالة النشر`,
  { label: "نشر Vercel" },
);
...
const deployUrl =
  deployResult &&
  (() => {
    const match = deployResult.match(
      /https:\/\/[a-zA-Z0-9.-]+\.vercel\.app[^\s]*/,
    );
    return match ? match[0] : null;
  })();

const pathsToVerify = Array.isArray(verifyPaths)
  ? verifyPaths
  : [verifyPaths || "/"];

const verifyResult = await agent(
  `تحقق من أن الموقع يعمل بعد النشر.
...
**المخرجات:** جدول: مسار | حالة | أخطاء | ملاحظات — ملخص ✅/⚠️/❌`,
  { label: "تحقق post-deploy" },
);
